...
- **Only weapons and armor have power.** Bungie hangs a `primaryStat` on plenty of things that have none — a sparrow's is Speed, a ghost's is Power Bonus, and a subclass gets a random armor stat that can read *negative*. Following [DIM](https://github.com/DestinyItemManager/DIM/blob/master/src/app/inventory/store/d2-item-factory.ts), `power` is reported only when that stat is Attack, Defense or Power; everything else has no power and is skipped by `is:haspower` and by `sort:power`.
