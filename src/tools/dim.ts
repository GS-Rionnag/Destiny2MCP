...
export function registerDimTools(server: McpServer): void {
  server.registerTool('dim_build', {
    description:
      ...
      + 'The notes are free text and often name gear or artifact columns the loadout itself does not carry — report them, do not treat them as equipment. When the notes name a seasonal artifact, believe artifact.liveArtifact and artifact.perksStillOnLiveArtifact over the notes: the saved perk hashes say what this build actually runs, an author\'s copy-pasted note does not. perksStillOnLiveArtifact counts PERKS whose exact hash is still on the live artifact — anti-Champion perks recur every season under the same name, so judge them by that count, never by whether a name sounds old.',
