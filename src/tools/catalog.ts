...
export function registerCatalogTools(server: McpServer): void {
  server.registerTool('search_items', {
    description:
      'Search EVERY item in the game (the whole Destiny manifest), not just the account\'s inventory — for designing builds around gear the player may not own. Same DIM query syntax as search_inventory: "is:handcannon is:legendary is:void", "is:exotic is:hunter is:helmet", "stat:range:>=70 is:pulserifle". perk: and is:godroll here mean CAN ROLL it, since a catalog item has no fixed roll: "is:handcannon perk:\'explosive payload\'" finds every hand cannon whose pool contains it, and "is:sniperrifle is:godroll" every sniper with a community wish-listed roll. Instance-only filters (power:, is:masterwork, is:locked, is:dupe) match nothing here — use search_inventory for those. Then call inspect_item for exact stats, the full perk pool and the god rolls.',
  ...
  server.registerTool('inspect_item', {
    description:
      'Everything the game knows about one item, owned or not: base stats, and every perk each socket column can roll (with descriptions) — the raw material for min-maxing a weapon or an armor piece. Also returns the community god rolls for it from the DIM wish list, aggregated: how many wish-listed rolls want each perk in each column, the most-wished perk combinations, and the reviewers\' notes explaining why. Answers "what is the god roll for X" for weapons the player has never seen. Takes a name or a hash from search_items.',
    inputSchema: z.object({
